...
ate_score_emb = pd.read_csv("ate_prop_score_emb.csv")
plt.figure(figsize=(10,4))
cmap = plt.get_cmap("cool")
x = ate_score_emb["Antidepressants"]
y = ate_score_emb["prop_score_ate_mean_emb"]
rescale = lambda y: (y - np.min(y)) / (np.max(y) - np.min(y))
plt.bar(x, y, color=cmap(rescale(y)), alpha=0.6)
plt.tick_params(axis='x', labelrotation=50)
plt.xlabel("Antidepressants")
plt.ylabel("ATE")
plt.tight_layout()
plt.show()
plt.savefig("ATE_on_Antidepressants_emb.png", dpi=300)